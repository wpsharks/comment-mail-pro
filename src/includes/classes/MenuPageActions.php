    ...
    protected function processQueue($request_args)
    {
        ...
        if (current_user_can($this->plugin->manage_cap)) {
            ...
            $notice_markup = // Notice regarding queue processor running successfully.
                sprintf(_n('A single queue-runner processed <code>%1$s</code> entry.', 'A single queue-runner processed <code>%1$s</code> entries.', $processed_entries, SLUG_TD), esc_html($processed_entries)).'<br />'.
                sprintf(__('<small><em><strong>Note:</strong> Each queue runner works for up to %1$s seconds and it will process up to %2$s queued emails. If additional emails remain in the queue (and they\'re ready to be sent) you can run again to continue processing.</em></small>', SLUG_TD), esc_html($this->plugin->options['queue_processor_max_time']), esc_html($this->plugin->options['queue_processor_max_limit']));
            $this->plugin->enqueueUserNotice($notice_markup, ['transient' => true]);
        }
