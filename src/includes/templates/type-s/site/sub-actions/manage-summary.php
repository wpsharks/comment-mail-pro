...
str_replace('%%title%%', __('My Comment Subscriptions', SLUG_TD), $site_header); ?>
                        ...
                        <li>
                            ...
                            switch ($_error_code) {
                                ...
                                case 'invalid_sub_key':
                                    echo __('Invalid subscription key; unable to display summary.', SLUG_TD);
                                    break; // Break switch handler.

                                default: // Anything else that is unexpected/unknown at this time.
                                    echo __('Unknown error; unable to display summary. Sorry!', SLUG_TD);
                            } ?>
                        </li>
                    <?php endforeach; ?>
                </ul>
        ...
        <?php else : // Display summary; there are no major errors. ?>
            ...
            <?php if ($processing && $processing_successes) : // Any processing successes? ?>
                ...
                <div class="alert alert-success">
                    <h4>
                        <?php echo __('Submission accepted; thank you :-)', SLUG_TD); ?>
                    </h4>
                    <ul class="list-unstyled">
            ...
            <h2 style="margin-top:0;">
                ...
                <?php /*
                <a href="<?php echo esc_attr($sub_new_url); ?>" title="<?php echo __('Add New Subscription', SLUG_TD); ?>">
                ...
                */?>
                ...
                <?php echo __('My Comment Subscriptions', SLUG_TD); ?><br />
            ...
            <?php if (empty($subs)) : ?>
                <h4>
                    <?php echo sprintf(__('No subscriptions at this time. You may <a href="%1$s">click here</a> to create one <i class="fa fa-smile-o" aria-hidden="true"></i>', SLUG_TD), esc_attr($sub_new_url)); ?>
                </h4>
            <?php endif; ?>
                                ...
                                <td>
                                    ...
                                    <div class="hover-links">
                                        <a href="<?php echo esc_attr($_sub_edit_url); ?>"
                                           title="<?php echo esc_attr(__('Edit Subscription', SLUG_TD)); ?>"
                                            ><i class="fa fa-pencil-square-o" aria-hidden="true"></i> <?php echo __('Edit Subscr.', SLUG_TD); ?></a>

                                        <span class="text-muted">|</span>

                                        <a data-action="<?php echo esc_attr($_sub_delete_url); ?>" href="<?php echo esc_attr($_sub_delete_url); ?>"
                                           data-confirmation="<?php echo esc_attr(__('Delete subscription? Are you sure?', SLUG_TD)); ?>"
                                           title="<?php echo esc_attr(__('Delete Subscription', SLUG_TD)); ?>" class="text-danger"
                                            ><?php echo __('Delete', SLUG_TD); ?> <i class="fa fa-times-circle" aria-hidden="true"></i></a>
                                    </div>
                                </td>
                                <td>
                                    <?php if ($_sub_post && $_sub_post_type_label) : ?>
                                        <?php echo sprintf(__('%1$s ID <a href="%2$s">#<code>%3$s</code></a> <a href="%4$s">%5$s</a>', SLUG_TD), esc_html($_sub_post_type_label), esc_attr($_sub_post_url), esc_html($_sub_post->ID), esc_attr($_sub_post_comments_url), esc_html($_sub_post_title_clip)); ?>
                                    <?php else : // Post no longer exists for whatever reason; display post ID only in this case. ?>
                                        <?php echo sprintf(__('Post ID #<code>%1$s</code>', SLUG_TD), esc_html($_sub->post_id)); ?>
                                    <?php endif; ?>
                                    ...
                                    <?php if ($_sub->comment_id) : ?><br />
                                        ...
                                        <?php if ($_sub_comment) : ?>
                                            <?php if ($_subscribed_to_own_comment) : ?>
                                                <?php echo sprintf(__('Replies to <a href="%1$s">your comment</a>; ID <a href="%1$s">#<code>%2$s</code></a> posted %3$s', SLUG_TD), esc_attr($_sub_comment_url), esc_html($_sub_comment->comment_ID), esc_html($_sub_comment_time_ago)); ?>
                                            <?php else : // It's not their own comment; i.e. it's by someone else. ?>
                                                <?php echo sprintf(__('Replies to <a href="%1$s">comment ID #<code>%2$s</code></a> posted %3$s', SLUG_TD), esc_attr($_sub_comment_url), esc_html($_sub_comment->comment_ID), esc_html($_sub_comment_time_ago)); ?>
                                            <?php endif; ?>
                                            <?php if ($_sub_comment->comment_author) : ?>
                                                <?php echo sprintf(__('by: <a href="%1$s">%2$s</a>', SLUG_TD), esc_attr($_sub_comment_url), esc_html($_sub_comment->comment_author)); ?>
                                            <?php endif; ?>
                                        <?php else : // Comment no longer exists for whatever reason; display comment ID only in this case. ?>
                                            <?php echo sprintf(__('Comment ID #<code>%1$s</code>', SLUG_TD), esc_html($_sub->comment_id)); ?>
                                        <?php endif; ?>
                ...
                <div class="row subs-pagination">
                    <div class="col-md-3 text-left">
                                <span class="label label-default" style="font-size:110%; vertical-align:bottom;">
                                    <?php echo sprintf(__('Page %1$s of %2$s', SLUG_TD), esc_html($pagination_vars->current_page), esc_html($pagination_vars->total_pages)); ?>
                                </span>
                    </div>
