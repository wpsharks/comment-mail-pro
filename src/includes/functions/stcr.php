...
function stcr_transition()
{
    ...
    $notice .= '<ul style="margin:0 0 0 3em; list-style:disc;">'.
               '   <li>'.sprintf(__('%1$s automatically imported many of your StCR options (to learn what was imported, see <a href="http://comment-mail.com/kb-article/stcr-options-transitioned-by-comment-mail/" target="_blank">this article</a>). It\'s still a good idea to review your %1$s configuration though.', SLUG_TD), esc_html(NAME)).'</li>'.
               '   <li>'.sprintf(__('<strong>IMPORTANT TIP:</strong> %1$s can import your existing StCR subscribers automatically too! <strong><a href="%2$s">Click here to review the StCR → %1$s import process</a></strong>.', SLUG_TD), esc_html(NAME), esc_attr($plugin->utils_url->importExportMenuPageOnly())).'</li>'.
               '</ul>';
